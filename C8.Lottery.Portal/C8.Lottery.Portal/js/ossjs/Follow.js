...
function joinhtml(data) {
    ...
    $.each(data.PageData, function (index, value) {
        var dl = "<dl class='details_DL'><dt><a href='/Personal/UserCenter/" + value.Followed_UserId + "'><img src='" + (value.Headpath.length <= 0 ? "/images/default_avater.png" : value.Headpath) + "'></a></dt>" + "<dd>" + "<div class='fans_ddRight f-r'  onclick='unorfollow(this," + value.Followed_UserId + ")'> <p class='fans_Qxgz'>取消关注</p></div>" + "<div class='details_ddLeft'>" + "<h3><a href='/Personal/UserCenter/" + value.Followed_UserId + "'>" + value.Nickname + "</a></h3>" + "<p>" + value.Autograph + "</p>" + "</div> </dd> </dl>";
        html.push(dl);
    });
    ...
    if (text == "关注") {
        $.post("/Personal/IFollow", {
            ...
            if (data.Success != true) {
                ...
                $(obj).append("<p class='fans_Qxgz'>取消关注</p>");
            }
        });
    } else {
        $(document).dialog({
