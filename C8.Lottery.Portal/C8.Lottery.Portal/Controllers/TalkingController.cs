...
namespace C8.Lottery.Portal.Controllers
{
    ...
    public class TalkingController : BaseController
    {
        ...
        private string[] GetSensitiveWordsList()
        {
            ...
            var str = "";
            if (CacheManager.GetObject<string>("GetSensitiveWordsList") == null)
            {
                string sql = " select content from SensitiveWords ";
                str = Convert.ToString(SqlHelper.ExecuteScalar(sql));

                CacheManager.AddObject("GetSensitiveWordsList", str, 2*60);
            }
            else
            {
                str = CacheManager.GetObject<string>("GetSensitiveWordsList");
            }
